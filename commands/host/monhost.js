  ...
  category: "Pokemon Raid",
    run: async (client, message, args) => {
       ...
       const Embed = new Discord.MessageEmbed()  
            .setColor('#c842f5')
            .setTitle('Locked Mon Host')
            .setThumbnail(message.author.displayAvatarURL())
            .setImage(`https://github.com/lbbooga/lbbooga.github.io/raw/master/anishiny/${args[0]}.gif`)
            .addFields(
              {name: 'Hosted By:', value: message.author, inline: true},
              {name: 'in room:', value: `${args[5]}`},
              {name: 'IV Spread', value: `${args[1]}`},
              {name: 'Gender', value: `${args[2]}`, inline: true},
              {name: 'Nature', value: `${args[3]}`, inline: true},
              {name: 'Ability', value: `${args[4]}`, inline: true},
