const ms = require("ms");
const { MessageEmbed } = require("discord.js");
const { Timers } = require("../../variable");
module.exports = {
  name: "timer",
  description: "Set a timer for your self!",
  usage: "<#d/h/m>",
  category: "utility",
  run: async (bot, message, args) => {
    if (!args[0]) {
     
      await message.delete(); 
          let Time = new MessageEmbed()
          .setDescription(`**Error** Sorry ${message.author.tag} but you need a number`)
          return message.channel.send(Time);
      
      /** return message.channel.send(
        `You did not specify the amount of time you wish to set a timer for!`
      ); 
*/
    }
    if (!args[0].endsWith("d")) {
      if (!args[0].endsWith("h")) {
        if (!args[0].endsWith("m")) {
          if (!args[0].endsWith("s")) {
          
            await message.delete(); 
          let EMBED = new MessageEmbed()
          .setDescription(`**ERROR** ${message.author.tag} That is not the right time i only do Days, Hours and Minuets so if you want a timer do: $timer (numer)(D/H/M/S)`)
          return message.channel.send(EMBED);

        }
      }
    }
  }
 
    if (isNaN(args[0][0])) {
      return message.channel.send(`That is not a number!`);
    }
    Timers.set(message.author.id + " G " + message.guild.name, {
      Guild: message.guild.name,
      Author: {
        Tag: message.author.tag,
        ID: message.author.id,
      },
      Time: ms(args[0]),
    });
   
    
      let Timer = new MessageEmbed() 
          .setDescription(`${message.author.tag} you have set a timer for ${args[0]} (${ms(args[0])}MS)`)
         // return
    message.channel.send(Timer);
    
       
    
    /**
    message.channel.send(
      `${message.author.tag} you have set a timer for ${args[0]} (${ms(args[0] )}MS)` );
    
  */
    await message.delete(); 
    setTimeout(() => {
      let Embed = new MessageEmbed()
        .setTitle(`Timer finished in guild ${message.guild.name}..`)
        .setDescription(
          `Your timer for ${args[0]} (${ms(args[0])}MS) has finished!`
        )
        .setColor(``);
      message.author.send(Embed);
      Timers.delete(message.author.id + " G " + message.guild.name);
    }, ms(args[0]));
      
  },
};